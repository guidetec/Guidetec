...
import guidetec.com.guidetec.adapter.CommerceAdapter
...
class CommerceFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        vista= inflater.inflate(R.layout.fragment_commerce, container, false)


        val orchardRecyclerView: RecyclerView =vista.findViewById(R.id.totalpartner_rv)
        orchardRecyclerView.setHasFixedSize(true)

        val orchardsL= Commerce.getCommerceFronFile(this.context,"commerce.json","commerce")
        val order=orchardsL.sortedWith(compareBy({ it.name }))

        val adapter = this.context?.let { CommerceAdapter(it, order) }
